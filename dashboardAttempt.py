...
def normalize_topic(topic):
    ...
    if isinstance(topic, str):
        ...
        topic = topic.replace("\\/", "/")
        # Add any other normalization steps as needed
    return topic

def preprocess_topics(df, mlb):
    """
    Convert assigned_topics from string representations of tuples to lists and normalize topics.
    """
    if df['assigned_topics'].dtype == object:
        # Safely evaluate the string to convert it into a list
        df = df.copy()  # To avoid SettingWithCopyWarning
        df['assigned_topics'] = df['assigned_topics'].apply(
            lambda x: list(ast.literal_eval(x)) if isinstance(x, str) else x
        )
    
    # Normalize topics
    df['assigned_topics'] = df['assigned_topics'].apply(lambda topics: [normalize_topic(t) for t in topics])
    return df
...
def calculate_topic_distribution(df, mlb, state=None):
    ...
    topic_counts = subset['assigned_topics'].explode().value_counts()
...
def create_ordered_sankey(national_dist, state_dist, topics, selected_state):
    ...
    fig.update_layout(title_text="Ordered Sankey Diagram with Consistent Topic Colors", font_size=10)
    return fig
...
def calculate_sentiment_per_topic(df, mlb, state=None):
    ...
    # Expand the topics
    subset_expanded = subset.explode('assigned_topics')
    
    # Group by topic and calculate mean sentiment
    sentiment_per_topic = subset_expanded.groupby('assigned_topics')['sentiment'].mean().reindex(mlb.classes_, fill_value=0)
...
def plot_sentiment_bar(national_sentiment, state_sentiment, topics):
    ...
    fig = px.bar(
        ...
        color='Category',
        barmode='group',
        title="Average Sentiment Score per Topic",
...
def calculate_topic_sentiment_correlation(df, mlb):
    ...
    topic_matrix = mlb.transform(df['assigned_topics'])
...
def get_text_for_topic(df, topic, state=None):
    ...
    """
    if state:
        subset = df[(df['assigned_topics'].apply(lambda topics: topic in topics)) & (df['state'] == state)]
    else:
        subset = df[df['assigned_topics'].apply(lambda topics: topic in topics)]
    # Assuming 'combined_text' contains the text data
...
def cluster_states(df, mlb, num_clusters=5):
    ...
    state_topic_dist = df.groupby('state')['assigned_topics'].apply(lambda topics: mlb.transform(topics).mean(axis=0))
...
def plot_state_clusters(cluster_df):
    ...
    fig = px.scatter_geo(
        ...
        scope='usa'  # Set scope to USA to make it a US map
    )
    return fig
...
def calculate_sentiment_heatmap(df, mlb):
    ...
    """
    # Expand the topics
    subset_expanded = df.explode('assigned_topics')
    
    # Group by state and topic, then calculate mean sentiment
    sentiment_heatmap = subset_expanded.groupby(['state', 'assigned_topics'])['sentiment'].mean().unstack(fill_value=0)
...
def main():
    ...
    csv_path = st.sidebar.text_input("Path to Labeled CSV", "labeled_preprocessed_reddit_posts_unlabeled_BERTOPIC.csv")
    ...
    required_columns = ['assigned_topics', 'state', 'combined_text']
    for col in required_columns:
        if col not in df.columns:
            st.error(f"The DataFrame does not contain the required '{col}' column.")
            st.stop()

    # Load or Fit MultiLabelBinarizer
    mlb = load_or_refit_mlbbinarizer(df)

    # Preprocess Topics
    df = preprocess_topics(df, mlb)
    ...
    if df['state'].dtype == object:
        # Assuming states are in CamelCase with no spaces
        df['state'] = df['state'].map(state_abbrev_dict)
        if df['state'].isnull().any():
            st.warning("Some state names could not be converted to abbreviations and are set to NaN.")
    ...
    # Additional Visualizations: Side-by-Side Bar Charts (Display All Topics)
    st.markdown("#### National vs State Topic Distribution")
    ...
    # Additional Visualizations: Heatmap
    ...
    st.markdown("### Top 5 More Prevalent Topics in Selected State")
    ...
    st.markdown("### Top 5 Less Prevalent Topics in Selected State")
    ...
    if sentiment_option and 'sentiment' in df.columns:
        ...
        # Sentiment Statistics Raw Data
        st.subheader("Sentiment Statistics Raw Data")
        sentiment_stats = pd.DataFrame({
            'Statistic': ['Mean', 'Median', 'Std Dev'],
            'National': [national_sentiment.mean(), national_sentiment.median(), national_sentiment.std()],
            selected_state: [state_sentiment.mean(), state_sentiment.median(), state_sentiment.std()]
        })
        st.write(sentiment_stats)

        # Display Sentiment Statistics with Correct Formatting
        sentiment_stats_formatted = sentiment_stats.copy()
        numeric_columns = ['National', selected_state]
        sentiment_stats_formatted[numeric_columns] = sentiment_stats_formatted[numeric_columns].applymap("{:.4f}".format)
        st.subheader("Sentiment Statistics")
        st.table(sentiment_stats_formatted)
        ...
        # New Section: Sentiment Prevalence
        st.markdown("### Sentiment Prevalence in Selected State Compared to National")
        ...
        st.markdown("### Top 5 Topics with Higher Sentiment in Selected State")
        ...
        st.markdown("### Top 5 Topics with Lower Sentiment in Selected State")
    ...
    # Geographical Choropleth Map
        ...
        if selected_topic_map:
            ...
            with st.spinner("Calculating sentiment distribution for selected topic..."):
                ...
                sentiment_per_state = df[df['assigned_topics'].apply(lambda topics: selected_topic_map in topics)].groupby('state')['sentiment'].mean()
    ...
    # Optionally, you can display word clouds for top N topics
        ...
        st.markdown("---")
        st.markdown("### About")
        st.markdown("This dashboard visualizes the distribution of topics and sentiment across different states compared to the national average. Explore various analytical dimensions to gain deeper insights.")
